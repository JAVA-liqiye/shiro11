...
//        String password = user.getPassword();
        //加密后密码
        String passwordMD5 = cryptographyUtil.md5(password,source);
        System.out.println(passwordMD5);
//        判断用户是否存在 不存在则抛出异常
        if (user != null) {
            //数据库密码
            String userPassword = user.getPassword();
            String userPaddwordMD5 = cryptographyUtil.md5(userPassword,source);
            System.out.println(userPaddwordMD5);
//            判断用户密码是否匹配 匹配则不匹配则抛出异常
//            if(user.getPassword().equals(password)){
            if (userPaddwordMD5.equals(passwordMD5)) {
